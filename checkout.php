...
// 1. Protect the page: Redirect to login if the user is not logged in
...
$malTotal      = floatval($_GET['malaysianTotal']  ?? 0);
$nonMalTotal   = floatval($_GET['nonMalaysianTotal'] ?? 0);
...
<body>
  ...
  <div id="infoBox">
	...
	<form id="checkOutForm" action="payment.php" method="post">
		...
		<input type="hidden" name="adult" value="<?php echo htmlspecialchars($adultCount); ?>">
		<input type="hidden" name="youth" value="<?php echo htmlspecialchars($youthCount); ?>">
		<input type="hidden" name="child" value="<?php echo htmlspecialchars($childCount); ?>">
		<input type="hidden" name="foreign" value="<?php echo htmlspecialchars($foreignCount); ?>">
		<input type="hidden" name="malaysianTotal" value="<?php echo htmlspecialchars($malTotal); ?>">
		<input type="hidden" name="nonMalaysianTotal" value="<?php echo htmlspecialchars($nonMalTotal); ?>">
		<input type="hidden" name="visitDate" value="<?php echo htmlspecialchars($visitDateRaw); ?>">
	...
	<br><br>
		...
		<small> By continuing, you agree to our <a href="terms_of_service.html">Terms of Service</a> and <a href="privacy_policy.html">Privacy Policy</a>.</small>
		<small>We’ll only contact you with essential updates or changes to your booking</small><br><br>
	  	<button type="submit">Continue to Payment</button><br><br>

		<small>You may receive occasional promotional emails from us related to your booked activities or other services of GetYourGuide. 
			Opt out by clicking here or at any time, as detailed in our Privacy Policy.</small>
	</form>
  </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      form.addEventListener('submit', function(e) {
        ...
        if (fullName.value.trim().length < 2) {
          showError(fullName, 'Please enter your full name (min 2 characters).');
          valid = false;
        } else {
          clearError(fullName);
        }

        // Email: HTML5 does basic, but extra regex if you like
        ...
        if (!phoneCode.value) {
          showError(phoneCode, 'Select your country code.');
          valid = false;
        } else {
        ...
        if (!phoneRe.test(phone.value.trim())) {
          showError(phone, 'Enter 6–15 digits only.');
          valid = false;
        } else {
          clearError(phone);
        }
